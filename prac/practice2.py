...
def draw_fingers(screen, highlighted_finger=None):
    for finger, position in finger_positions.items():
        color = RED if finger == highlighted_finger else BLACK
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_ESCAPE:
                ...
                highlighted_finger = finger_mapping.get(event.unicode, None)
    ...
    # 손가락 강조 화면에 표시
    draw_fingers(screen, highlighted_finger)
